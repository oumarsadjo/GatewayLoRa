...
void applicationGPS();                     // Pour les applications GSM
void readMessage();
...
//----------------------------------- D E B U T  -- G S M ---------------------------------//
void applicationGPS() {
  sendCommand("AT+CGNSMOD=?");        // 1. Verifier les commandes AT du GSM
  sendCommand("AT+CGNSMOD=1,1,1,1");  // 2. Config le mode GNSS
  sendCommand("AT+CGNSCFG=1");        // 3. Config de la sortie des donnees
  sendCommand("AT+CGNSTST=1");        // 4. Fixation des satellites disponibles
  sendCommand("AT+CGPSSTATUS");       // 5. Affichage des satellites trouves
  sendCommand("AT+CGNSINF");          // 6. Affichage : Latitude et Longitude
}
//-------------------------------------- F I N  -- G S M ---------------------------------//
//
